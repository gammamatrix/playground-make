...
/**
 * \Playground\Make\Configuration\Model\Concerns\Attributes
 */
trait Attributes
{
    /**
    ...
    public function addAttribute(
        ...
        if (empty($column) || ! is_string($column)) {
            throw new \RuntimeException(__('playground-make::model.Attributes.invalid', [
    ...
    public function addCast(
        ...
        if (empty($column) || ! is_string($column)) {
            throw new \RuntimeException(__('playground-make::model.Casts.invalid', [
                'name' => $this->name() ?: 'model',
                'column' => is_string($column) ? $column : gettype($column),
            ]));
        }
    ...
    {
        if (empty($column) || ! is_string($column)) {
            throw new \RuntimeException(__('playground-make::model.Fillable.invalid', [
                'name' => $this->name() ?: 'model',
                'column' => is_string($column) ? $column : gettype($column),
            ]));
        }
